...
use hyper::{Body, Client, Method, Request, Uri, client::connect::HttpConnector, Response, server::conn::Http};
use hyper_tls::HttpsConnector;
use tokio::net::TcpStream;

use sui_json_rpc_types::{SuiData, SuiObjectDataOptions, SuiObjectResponse, SuiRawData, SuiObjectData, SuiParsedData, SuiParsedMoveObject, SuiMoveStruct, SuiMoveValue};
use sui_transaction_builder::DataReader;
use sui_types::{base_types::{ObjectID, ObjectInfo, ObjectRef, ObjectType, SuiAddress, SUI_ADDRESS_LENGTH, SequenceNumber, MoveObjectType}, digests::{ObjectDigest, TransactionDigest}, object::Owner, TypeTag, id::ID};
...
fn parse_object_from_response(input: Body) -> SuiObjectResponse {
    ...
    let fake_object_data = SuiObjectData { 
        object_id: ObjectID::from_hex_literal("0xbd4a8807df7b05c6f4569f3ef92c05ea38ea2e1eaac2455b7412fd8953f82fcd").unwrap(),
        version: SequenceNumber::from_u64(48138),
        digest: ObjectDigest::from_str("9nMCTJvv9ftWQCc1qVEpSzNAGtFunVDpGk2xiBjjhJHZ").unwrap(),
        type_: Some(ObjectType::Struct(sui_types::base_types::MoveObjectType::Coin(sui_types::TypeTag::Address))),
        owner: Some(Owner::AddressOwner(SuiAddress::from_str("0xfc8238ec72483cc0ca7dd86296f4cebaa15fc4f8207521b1710f7403db010a8f").unwrap())),
        previous_transaction: Some(TransactionDigest::from_str("CZUByEwbToqhZQ5FcnyYEWFoikZHvVSAdPPRcriFWM32").unwrap()),
    ...
    SuiObjectResponse { data: Some(fake_object_data), error: None }
}
...
//    let objects = serde_json::from_str(input);
    ...
    let fake_struct_tag = StructTag {
        address: AccountAddress::from_hex_literal("0xfc8238ec72483cc0ca7dd86296f4cebaa15fc4f8207521b1710f7403db010a8f").unwrap(),
    ...
    let fake_object_info = ObjectInfo {
        object_id: ObjectID::from_hex_literal("0x0013dadc29fcab80c84a30670e174851a3df95f4ae26ed8445a52fcc0267769a").unwrap(),
